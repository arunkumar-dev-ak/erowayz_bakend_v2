...
@UseGuards(AuthGuard)
export class ServiceBookingGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  private readonly logger = new Logger(ServiceBookingGateway.name);
  ...
  async handleDisconnect(client: Socket) {
    ...
    await Promise.all([
      this.redisService.srem(`service-booking-sockets:${userId}`, socketId),
      this.redisService.rem(`service-booking-user:${socketId}`),
    ]);

    const remainingSockets = await this.redisService.scard(
      `service-booking-sockets:${userId}`,
    );
    if (remainingSockets === 0) {
      await this.redisService.rem(`service-booking-sockets:${userId}`);
    }
